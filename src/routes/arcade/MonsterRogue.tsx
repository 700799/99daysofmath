...
export function MonsterRogue() {
  ...
  const [special, setSpecial] = useState<Challenge | null>(null);
  ...
  const doSpecial = () => {
    if (!foe || busy) return;
    setSpecial(makeChallenge(Math.min(5, 1 + Math.floor(wave / 3))));
    setSpecialInput('');
  };
  const resolveSpecial = () => {
    if (!special || !foe) return;
    const ok = Number(specialInput.trim()) === special.answer && specialInput.trim() !== '';
    setSpecial(null);
    const a = party[active];
  ...
  const reset = () => {
    ...
    setBusy(false); setSpecial(null); setSwapOpen(false); setOutcome(null);
    setPhase('howto');
  };
      ...
      <div>
        ...
        <HowToPlay
          ...
          sections={[
            ...
            { heading: 'Your turn', body: 'Attack = normal hit. Special = a quick math problem; answer right for a CRITICAL (×2)! Catch = add a weakened wild critter to your party (up to 4). Swap = change your active critter.' },
          ...
          controls="Tap the action buttons. Special problems use the on-screen keypad."
      ...
      {special && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 p-4">
          <div className="w-full max-w-xs rounded-3xl bg-white p-5 text-center shadow-2xl">
            ...
            <div className="mt-1 font-display font-extrabold text-slate-900">Special! Solve for a CRITICAL hit:</div>
            <div className="mt-3 rounded-2xl bg-slate-50 border-2 border-slate-200 py-4 text-2xl font-display font-extrabold tabular-nums">{special.prompt}</div>
            <input autoFocus inputMode="numeric" value={specialInput} onChange={(e) => setSpecialInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && resolveSpecial()}
            ...
            <button type="button" onClick={resolveSpecial} className="mt-3 w-full min-h-11 rounded-2xl bg-violet-500 text-white font-display font-extrabold">Unleash ✨</button>
          </div>
        </div>
      )}
    </div>
